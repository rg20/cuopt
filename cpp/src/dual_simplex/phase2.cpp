                               ...
                               f_t& work_estimate)
{
  ...
  for (i_t h = 0; h < scaled_delta_xB_nz; ++h) {
    ...
    if (k == basic_leaving_index) {
      ...
      const f_t cross_term = quadratic ? 0.0 : 2.0 * wk * v[k] / wr;
      f_t new_val          = delta_y_steepest_edge[j] + cross_term + wk * wk * omegar;
                                              ...
                                              work_limit_context_t* work_unit_context)
{
  ...
  if (phase == 2) {
    settings.log.printf("Dual steepest-edge weight update: %s\n",
                        settings.pricing_strategy == pricing_strategy_t::QUADRATIC_STEEPEST_EDGE
                          ? "quadratic"
                          : "exact");
